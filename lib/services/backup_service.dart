...
class BackupService {
  ...
  static Future<List<BackupInfo>> getAutomaticBackups() async {
    try {
      final directory = await getApplicationDocumentsDirectory();
  ...
  static Future<void> createAutomaticBackup() async {
    try {
      final directory = await getApplicationDocumentsDirectory();
      final backupDir = Directory('${directory.path}/backups');
      ...
      // Create backup
      final tempBackup = await createBackup();
      if (tempBackup != null) {
        final timestamp = DateTime.now().millisecondsSinceEpoch;
        final fileName = 'tailor_shop_backup_$timestamp.json';
        ...
        await File(tempBackup).copy(targetPath);
  ...
  static Future<void> _cleanupOldBackups(Directory backupDir) async {
    try {
      final backups = await getAutomaticBackups();
